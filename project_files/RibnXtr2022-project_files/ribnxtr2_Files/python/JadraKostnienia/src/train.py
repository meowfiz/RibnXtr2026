...
def main():
    ...
    output_dir_path = experiment_root / "output"
    ...
    if args.resume_best:
        file_to_load = output_dir_path / "best_checkpoint.pth"
        
    elif args.resume_last:
        file_to_load = output_dir_path / "last_checkpoint.pth"
    ...
    #TODO: do usuniecia w przyszlych iteracjach, albo do umieszczenia jako flaga --reload-config czy coś takiego
    #obecnie może wystąpić przy niektórych checkpointach, bo przypadkiem nie zapisywałem configu
    if len(config) == 0:
        config = load_config(experiment_root / "resolved_config.yaml")
    ...
    #setup 
    train_dataset, validation_dataset = build_dataset(config['dataset'], Path("D:/") / "Praca" / "JadraKostnienia")
    ...
    loss_function = build_loss(config['loss'])
    optimizer = build_optimizer(config['optimizer'], model)

    num_epochs = config['base']['epochs']
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        with torch.no_grad():
            for images, masks in validation_loader:
                ...
                #TODO: liczenie metryk
                #TODO: zapisywanie metryk
                #TODO: 
        ...
        current_checkpoint = CheckpointData(loaded_checkpoint.experiment_name, epoch, model, optimizer, losses, config)
        checkpoint = current_checkpoint.save_to_dict()
          
        torch.save(checkpoint, output_dir_path / "last_checkpoint.pth")

        if new_best:
            torch.save(checkpoint, output_dir_path / "best_checkpoint.pth")
